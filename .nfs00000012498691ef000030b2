...
if [ "$GENOME" == "mm9" ]; then
    ...
    if [ ! -z "$STAR" -o ! -z "$STAR_SINGLE" ]; then
        GENOMEINDEX="/home/xfd783/genomes/assemblies/Dro_melanogaster/dm6/STAR"
    elif [ ! -z "$KALLISTO" ]; then
    ...
    if [ ! -z "$STAR" -o ! -z "$STAR_SINGLE" ]; then
        GENOMEINDEX="/home/xfd783/genomes/assemblies/spikeIn/mm10_dm6/STAR"
    elif [ ! -z "$KALLISTO" ]; then
        GENOMEINDEX="/home/xfd783/genomes/assemblies/spikeIn/mm10_dm6/kallisto/"
    else
        GENOMEINDEX="/home/xfd783/genomes/assemblies/spikeIn/mm10_dm6/bowtie2/Bowtie2IndexWithAbundance"
    fi
elif [ "$GENOME" == "mm10_ecoli" ]; then
    if [ ! -z "$STAR" -o ! -z "$STAR_SINGLE" ]; then
        GENOMEINDEX="/home/xfd783/genomes/assemblies/spikeIn/mm10_ecoli/STAR"
    elif [ ! -z "$KALLISTO" ]; then
        GENOMEINDEX="/home/xfd783/genomes/assemblies/spikeIn/mm10_ecoli/kallisto/"
    else
        GENOMEINDEX="/home/xfd783/genomes/assemblies/spikeIn/mm10_ecoli/bowtie2/Bowtie2IndexWithAbundance"
    fi
...
if [ ! -z "$STAR_SINGLE" ]; then
    ...
    if [ -z "$BAMTOBW" ]; then
        ...
        # ls *.sort | parallel -j 1 'bedGraphToBigWig {} /home/xfd783/genomes/annotations/SIZE/mouse.mm9.genome bigWig/{.}.bw' &
        if [ ! -z "$UNIQUE" ]; then
